...
let frameworkSegmentBasenames: ReadonlySet<string> = new Set([
  "page.tsx",
  "layout.tsx",
]);

const _isFrameworkSegmentFile = ({ filename }: { filename: string }) =>
  frameworkSegmentBasenames.has(
...
const _exportsFromDeclaration = ({
  ...
  if (declaration?.type === "FunctionDeclaration") {
    ...
    return binding === undefined
      ? []
      : _functionExport({ binding, name: binding.name, node });
  }
  ...
  return _nodeArray({ field: "declarations", node: declaration }).flatMap(
    (variable) => {
...
const _exportsFromDefaultDeclaration = ({
  ...
  if (declaration.type === "Identifier") {
    ...
    return binding === undefined
      ? []
      : _functionExport({ binding, name, node });
  }
