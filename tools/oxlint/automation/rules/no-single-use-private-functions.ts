...
const isEffectCall = ({
  ...
  effectCalleeName({ node: nodeField({ field: "callee", node }) ?? node }) ===
    name;
      ...
      addIdentifierName({
        ...
        node:
          declaration === undefined
            ? undefined
            : nodeField({ field: "id", node: declaration }),
      });
    }
...
const runtimeReadCount = ({
  references,
}: {
  references: Array<ReferenceLike>;
}) =>
  references.filter((reference) => isRuntimeReadReference({ reference }))
    .length;
...
const rule: Rule = {
  ...
  create(context) {
    return {
      Program(node) {
        type SourceNode = Parameters<
          typeof context.sourceCode.getDeclaredVariables
        >[0];
        type ScopeVariable = ReturnType<
          typeof context.sourceCode.getDeclaredVariables
        >[number];
        type Candidate = {
          kind: CandidateKind;
          ...
          if (statement.type === "VariableDeclaration") {
            return statement.declarations.flatMap(
              (declaration): Array<Candidate> => {
                ...
                return variable === undefined
                  ? []
                  : [{ kind, name, node: declaration, variable }];
              }
            );
          }
