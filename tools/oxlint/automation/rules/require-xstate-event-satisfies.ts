...
const unwrapExpression = ({ node }: { node: Node }): Node => {
  if (
    ...
    return !isNode(node.expression)
      ? node
      : unwrapExpression({ node: node.expression });
  }
...
const isSatisfiedObjectEvent = ({ node }: { node: Node }) => {
  ...
  return (
    ...
    unwrapExpression({ node: expression.expression }).type ===
      "ObjectExpression"
  );
};
...
const rule: Rule = {
  ...
  create(context) {
    const collectEventObjectsMissingSatisfies = ({
      node,
    }: {
      node: Node;
    }): Array<Node> => {
      ...
      if (
        ...
        if (
          ...
          return nodeList({ value: eventFactory.body.body }).flatMap(
            (statement) => {
              if (
