...
export const propertyValue = ({ node }: { node: Node }) =>
  nodeField({ field: "value", node });
...
export const collectSetupDefinitions = ({
  ...
  const definitionsObject =
    definitionsProperty === undefined
      ? undefined
      : propertyValue({ node: definitionsProperty });
  ...
  return nodeList({ value: definitionsObject.properties }).flatMap(
    (property): Array<Definition> => {
      ...
      return name !== undefined && isReportNode(property)
        ? [{ name, node: property }]
        : [];
    }
  );
...
export const typedReferenceName = ({ node }: { node: Node }) => {
  ...
  const typeValue =
    typeProperty === undefined
      ? undefined
      : propertyValue({ node: typeProperty });

  return typeValue === undefined
    ? undefined
    : stringLiteralValue({ node: typeValue });
};
...
export const setupCallFromCreateMachineCall = ({ node }: { node: Node }) => {
  ...
  const setupCallee =
    setupCall === undefined
      ? undefined
      : nodeField({ field: "callee", node: setupCall });
